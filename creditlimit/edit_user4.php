...
<Div class="col-md-12" id="middle">
	<div class="container">
		...
		$query = "UPDATE LOG_DATA SET Status = 0  where TMP = '$TMP'";
		...
		// 			WHERE (Status = 1)) x group by DealerCode) x on x.DealerCode=debcode where debnr is not NULL and ltrim(debcode) = '$DealerCode' ";
		   ...
		   $query10 = "UPDATE cicmpy SET CreditLine = Amount 

				FROM cicmpy inner join 
				(SELECT DealerCode,isnull(sum(amount),1) Amount 
				FROM (SELECT DealerCode, LOG as LogtLimit,TemporaryCreditLimit as TemporaryCreditLimit ,Status,
				Case when Status = 1 then 
				case when  TemporaryCreditLimit>0 then TemporaryCreditLimit else Log end 
				else null ENd Amount
				FROM  LOG_DATA
				) x group by DealerCode) x on x.DealerCode=debcode where debnr is not NULL and ltrim(debcode) = '$DealerCode'";

		$run10 = sqlsrv_query($conn,$query10);

				echo "<script> window.alert('გაუქმებულია!');
